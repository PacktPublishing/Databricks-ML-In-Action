...
## Reference Guide
Below is a visual overview of the MLflow Classification Recipe's information flow.
...
- `target_col`: string. Required. MUST have a cardinality of two.
...
### Ingest step
The ingest step loads the dataset specified by the `steps.ingest` section in [`recipe.yaml`](https://github.com/mlflow/recipes-classification-template/blob/main/recipe.yaml). 
...
Example loader_method function for [`ingest.py`](https://github.com/mlflow/recipes-classisification-template/blob/main/steps/ingest.py)
```python
...
<u>Example</u>:
  ...
  Example config in [`recipe.yaml`](https://github.com/mlflow/recipes-classification-template/blob/main/recipe.yaml):
  ```
  steps:
    split:
      using: custom
      ...
      post_split_filter_method: create_dataset_filter
  ```
  Example create_dataset_filter function for [`split.py`](https://github.com/mlflow/recipes-classification-template/blob/main/steps/split.py)
  ```python
  def custom_split(df):
    """
...
<u>Example</u>:
  ...
  Example transformer_fn function for [`transform.py`](https://github.com/mlflow/recipes-classification-template/blob/main/steps/transform.py)
  ```python
...
<summary><strong><u>Using: "custom"</u></strong></summary>
  ...
  - `predict_scores_for_all_classes`: boolean. Optional. Defaults to `True`
    ...
    <u>Output Example</u>:
      ...
      Assume that positive_class: `1` and other class is `0`, below looks like an output for computed probability of all the classes.

      | predicted_score_0 | predicted_score_1 | predicted_score | predicted_label |
      | ----------------- | ----------------- | --------------- | --------------- |
      | 0.999858          | 0.000142          | 0.999858        | 0               |
      | 0.013228          | 0.986772	        | 0.986772        | 1               |
      | 0.997725          | 0.002275          | 0.997725        | 0               |

  - `predict_prefix`: boolean. String. Defaults to `predicted_`
    prefix string to prefix the column with predicted scores and label. In the above example, we can see `predicted_` as default.
  ...
  Example estimator_fn function for [`train.py`](https://github.com/mlflow/recipes-classisification-template/blob/main/steps/train.py)
  ```python
...
### Batch scoring
After model training, the classification recipe provides the capability to score data with the
...
Example experiment config in [`profile.yaml`](https://github.com/mlflow/recipes-classisification-template/tree/main/profiles)
  ```
  experiment:
...
#### Built-in metrics
The following metrics are built-in. Note that `greater_is_better = True` for all these metrics except `false_positives` and `false_negatives`:
...
Custom evaluation metric functions should be defined in [`steps/custom_metrics.py`](https://github.com/mlflow/recipes-classisification-template/blob/main/steps/custom_metrics.py). Function to define the custom_metrics should adhere to the following [signature](https://www.mlflow.org/docs/latest/python_api/mlflow.models.html#mlflow.models.EvaluationMetric).
